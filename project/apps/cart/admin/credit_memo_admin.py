...
from ..models import CreditMemo, CreditMemoItem
from ..utils import helper


class CreditMemoItemInline(admin.TabularInline):
...
class CreditMemoTotalsInline(admin.StackedInline):
    from django.utils.datastructures import ImmutableList
    from django.db import models

    verbose_name = "Credit Memo"
    verbose_name_plural = "Credit Memo Totals"
    model = CreditMemo
    extra = 0
    can_delete = False
...
class CreditMemoAdmin(NonSortableParentAdmin):
    list_display = (
        'id',
        'increment_id',
        'order_increment_id',
    ...
    readonly_fields = ('subtotal', 'shipping_amount', 'tax_amount','grand_total')
    list_filter = ('created_at', )

    inlines = (
        CreditMemoItemInline,
        CreditMemoTotalsInline,
    )
    fieldsets = (
        (
            'BASIC INFORMATION', {
                'fields': (
                    ('increment_id', 'credit_memo_date'),
                    ('order_increment_id',),
                ),
            }),
        (
