...
class Invoice(models.Model):
    ...
    invoice = models.ForeignKey('self', on_delete=models.CASCADE, blank=True, null=True, default=None, related_name='self')
    ...
    def __str__(self):
        return 'Invoice #{} with Total of: {} for Order #{}'.format(self.get_invoice_id(), self.grand_total,self.order.get_order_id())

    def get_invoice_id(self):
        return self.increment_id if self.increment_id else self.id

    def get_view_url(self):
        return reverse('cart:account_order_invoice_list', self.order.id)
    ...
    def fetch_new_increment_id():
        ...
        last_id = global_config.invoice_increment_last_id if global_config.invoice_increment_last_id else 1
        ...
        global_config.invoice_increment_last_id = new_id
        global_config.save()
    ...
    def can_cancel(self):
        return self.status not in ['refunded']
