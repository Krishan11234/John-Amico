...
<section class="body">
    ...
    <?php if( $display_header ) :?>
        ...
        <?php
            $user_full_name = '';
            if(!empty($_SESSION['member']['ses_member_first_name'])) { $user_full_name .= ucfirst( strtolower($_SESSION['member']['ses_member_first_name']) ) . ' '; }
            if(!empty($_SESSION['member']['ses_member_last_name'])) { $user_full_name .= ucfirst( strtolower($_SESSION['member']['ses_member_last_name']) ); }

            $userType = "Member";
            if($_SESSION['member']['mtype'] == 'a') {
                $userType = "Professional";
            }
            elseif($_SESSION['member']['mtype'] == 'm') {
                $userType = "Professional Pro";
            } elseif($_SESSION['member']['mtype'] == 'e') {
                $userType = "EC";
            }
            elseif($_SESSION['member']['mtype'] == 'c') {
                $userType = "Chapter";
            }
        ?>
                    ...
                    <div id="userbox" class="userbox">
                        ...
                        <div class="dropdown-menu">
                            <ul class="list-unstyled">
                                ...
                                <li>
                                    <a role="menuitem" tabindex="-1" href="<?php echo base_member_url(); ?>/logout.php"><i class="fa fa-power-off"></i> Logout</a>
                                </li>
                            </ul>
                        </div>
                    </div>
