<?php
$page_name = 'Business Services Information';
...
if($user_mtype == "e" || $user_mtype == "m") {
    if ($_POST['goto'] == 'save__cinfo_business_service') {
        $query = "delete from  extra_b_customers where customers_id='$customers_id'";
        mysqli_query($conn, $query);

        foreach ($_POST['field'] as $key => $val) {
            $query = "insert into extra_b_customers set field_id='$key', customers_id='$customers_id', val='$val' ";
            mysqli_query($conn, $query);
        }
    ...
    if ($_POST['goto_notes'] == 'save__cinfo_business_service') {

        foreach ($_POST['fields'] as $key => $val) {
            $query = "delete from  extra_b_notes where customers_id='$customers_id' and item_id='{$val['item_id']}'and SKOEInvoice='{$val['SKOEInvoice']}'";
            mysqli_query($conn, $query);

            $query = "insert into extra_b_notes set customers_id='$customers_id', item_id='{$val['item_id']}', SKOEInvoice='{$val['SKOEInvoice']}', notes='{$val['notes']}', date='{$val['date']}'";
            mysqli_query($conn, $query);
        }
                ...
                <div class="panel-body">
                    <div class="row">
                        <div class="col-xs-12 profile ">
                            ...
                            <?php
                            $query = "select id, title from extra_business_systems where title is not null and title!='' and item_id = '' order by id ";
                            ...
                            if( mysqli_num_rows($q) ) :
                                ...
                                ?>
                                <div class="col-md-4">
                                    <form name="cinfo_business_service" id="cinfo_business_service" action="" method="post">
                                        <section class="panel panel-primary">
                                            <header class="panel-heading"><h2 class="panel-title text-center">Customer Fields</h2></header>
                                            <div class="panel-body">
                                                <input type="hidden" name="memberid" value="<?=$memberid?>" >
                                                <input type="hidden" name="goto" value="save__cinfo_business_service">

                                                <?php
                                                while ($row = mysqli_fetch_array($q)) :
                                                    $aaa = mysqli_fetch_array(mysqli_query($conn,"select val from extra_b_customers where field_id='".$row['id']."' and customers_id='$customers_id'"));
                                                    ?>
                                ...
                                <div class="<?php echo (!empty($first_protion_loaded) ? 'col-md-8' : 'col-md-12') ?>">
                                    <section class="panel panel-primary">
                                        <form name="cinfo" id="cinfo" action="" method="post">
                                            ...
                                            <div class="panel-body">
                                                <input type="hidden" name="memberid" value="<?=$memberid?>" >
                                                <input type="hidden" name="goto_notes" value="save__cinfo_business_service">
                                                ...
                                                <?php if($user_mtype == "e" || $user_mtype == "m") : ?>
                                                    <div class="table-responsive">
                                                        <table class="table table-bordered table-striped">
                                                            ...
                                                            <?php
                                                            $query = "select * from extra_business_systems where title is not null and title!='' and item_id != '' order by category ASC ";
                                                            $q = mysqli_query($conn,$query);

                                                            $i=0;
                                                            while ($row = mysqli_fetch_array($q)) :
                                                                $line_items_sql = "
                                                                    SELECT bwli.*, bwi.InvoiceDate, bwi.SKOEInvoice as SKOEI, ebn.*
                                                                    ...
                                                                    LEFT JOIN extra_b_notes ebn ON ebn.SKOEInvoice = bwi.SKOEInvoice
                                                            ...
                                                            <?php else :?>
                                                                <?php
                                                                $line_items = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM extra_b_notes ebn WHERE customers_id = '$customers_id' and item_id='{$row['item_id']}' and SKOEInvoice = 0 ORDER BY id DESC LIMIT 0,1"));
                                                ...
                                                <?php elseif( $user_mtype == 'c' ) :?>
                                                    <?php
                                                    $query = "select title, id from extra_business_systems where title is not null and title!='' order by id ";
                                                    $q = mysqli_query($conn,$query);
