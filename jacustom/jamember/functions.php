...
function members_to_chapter($customer_id) {
    ...
    if ($f['mtype'] == 'c') {
        ...
        if (mysqli_num_rows($query_zip) == 0) {
            ...
            $query = mysqli_query($conn, "SELECT * FROM tbl_member, address_book, zipdata WHERE tbl_member.mtype='m' AND tbl_member.int_customer_id=address_book.customers_id AND address_book.entry_postcode=zipdata.zipcode " . $add . " GROUP BY amico_id") or die (mysql_error());
