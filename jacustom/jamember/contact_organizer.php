...
function unsubscribe_link ($member_id = null, $subscription_member_id = null) {
    $unsubscribe_link_text = base_url()."/jamember/unsubscribe.php?member_id=".$member_id."&subscription_member_id=".$subscription_member_id."&validation_hash=".md5($member_id.UNSUBSCRIBE_STRING);
    $unsubscribe_link = '<br><br>You can <a style="color:#0d8db8; font-weight:bold;" href="'.$unsubscribe_link_text.'" target="_blank">Unsubscribe</a>. Please remember, if you "Unsubscribe", you will not receive any update in future.';
            ...
            switch ($email_to) {
                case 'downline':
                    ...
                    while ($rows = mysqli_fetch_array($results)) {
                        ...
                        $zag .= unsubscribe_link($member_id, $rows['int_member_id']);
                    ...
                    <div class="col-lg-8 col-md-8 col-xs-12 email_wrapper">
                        <form name="show_commissions" class="form form-validate form-bordered" action="" method="post">
                            ...
                            <footer class="panel-footer text-center">
                                ...
                                <?php if( !empty($_POST['goto']) && ($_POST['goto'] == 'send_email') && !empty($_POST['send_email']) && empty($error_message)) {  ?>
                                    <script>
                                        (function($){
                                            var target = $("input[type='submit']");
                                            target.css("background", "#ff0000");
                                            target.css("border", "none");
                                            target.css("border-color", "none");
                                            target.attr('disabled', 'disabled');
                                            setTimeout(function(){
                                                target.css("background", "#0088cc");
                                                target.css("border", "1px solid #0088cc");
