<?php
$page_name = 'Service Systems Extra Fields';
...
if( !empty($_POST['goto']) && ($_POST['goto'] == 'save')){
	...
	$query = "TRUNCATE TABLE extra_service_systems";
	mysqli_query($conn,$query);
	$query = "ALTER TABLE extra_service_systems AUTO_INCREMENT = '1'";
	mysqli_query($conn,$query);
	...
	foreach ($_POST['field']['title'] as $key => $val){
		...
		//$query = "update extra_service_systems set $key='$v' where id='$k'";
			if ($_POST['field']['title'][$i] != ''){	
				$query = "insert into extra_service_systems set title='".$_POST['field']['title'][$i]."', item_id='".$_POST['field']['item_id'][$i]."', category='".$_POST['field']['category'][$i]."'";
				mysqli_query($conn,$query);
			}
...
function addRow(frm) {
    ...
    //var row = '<p id="rowNum'+rowNum+'" class="rw">Category:<input type="Text" name="field[category]['+rowNum+']" size="20" maxlength="20" value=""> Field #'+rowNum+':<input type="Text" name="field[title]['+rowNum+']" size="20" maxlength="20" value=""> Item ID:<input type="Text" name="field[item_id]['+rowNum+']" size="20" maxlength="20" value=""> <input type="button" value="Remove" onclick="removeRow('+rowNum+');"></p>';
            ...
            <section class="panel">
                <div class="col-md-10 col-sm-12 col-xs-12 centering">
                    ...
                    <div class="panel-body system_fields">
                        <?php
                        $q = mysqli_query($conn,"select * from extra_service_systems order by category ASC");
