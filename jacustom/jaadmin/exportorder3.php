...
$reccount = 0;
$rsorder = mysqli_query($conn,"SELECT
						o.orders_id,
						o.customers_id, 
						o.customers_name, 
...
while (list($orders_id,
	    ...
	    $cc_expires,
		$date_purchased,
	    $order_status) = mysqli_fetch_row($rsorder)) {
	...
	$customers_name = str_replace(" ","",$customers_name);
	
	$customers_name = "c".$customers_name;
	
	$customers_name = substr($customers_name, 0, 11);

	//SECION FOR /SOHDR
	$Process="/"."";
	$Line="SOHDR".",";
	$CustomerID=$customers_name.",";
	$OrderStatus="O".",";
	$OrderNumber=$orders_id.",";
	$Description=$Note="Online Purchase: Non-Member".",";
	$PurchaseOrderNumber="".",";
	$QuoteDate="".",";
	...
	//END /SOSUM
		...
		//SOHDR
	$strrecord[$reccount]=$Process.$Line.$CustomerID.$OrderStatus.$OrderNumber.$Description.$PurchaseOrderNumber.$QuoteDate.$OrderDate.$RequiredDate.$ReleaseDate.$ShipVia.$FOB.$Terms.$IsTaxable.$SalesRep.$Note.$ConvertProbability.$ReasonForHold;
	$reccount=$reccount+1;
		//QUERY FOR PRODUCTS
...
$ft=fopen("./csv/orders_export/non-customer-order.txt","w");
...
DownloadFile("./csv/orders_export/non-customer-order.txt");

//Save a copy of this CSV file on the server.
$today = date("Ymd");
rename("./csv/orders_export/non-customer-order.txt", "./csv/orders_export/non-customer-order.txt.$today");
