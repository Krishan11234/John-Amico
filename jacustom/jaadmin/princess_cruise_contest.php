...
if($thisTableReady) {
    ...
    //$conditions = array("ir.updated_by_cron >= '{$reportGenerator->getStartTime()}'", "ir.updated_by_cron <= '{$reportGenerator→getEndTime()}'");
    $conditions = array("ir.updated_by_cron >= '{$reportGenerator->getStartTime()}'", "ir.updated_by_cron <= '{$reportGenerator->getEndTime()}'");
    if (!empty($conditions)) {
        if($reportGenerator->getIncentiveTypeColumnReady() && !empty($reportGenerator->getIncentiveType()) ) {
            $sql .= " AND ";
    ...
    // $sql .= " AND ir.updated_by_cron >= '{$reportGenerator->getStartTime()} AND ir.updated_by_cron <= '{$reportGenerator→getEndTime()}' ";
