...
$member_type_name = 'Chapter';
$member_type_name_plural = 'Chapters';
$self_page = 'chap.php';
$page_url = base_admin_url() . '/chap.php?1=1';
$action_page = 'act_members.php';
$action_page_url = base_admin_url() . "/$self_page?1=1";
$export_url = base_admin_url() . '/chap_export.php';
...
$alpabet = (!empty($_REQUEST['alpabet']) ? filter_var($_REQUEST['alpabet'], FILTER_SANITIZE_STRING) : 'ALL');
...
if( (!empty($_POST['memberid']) && !empty($_POST['goto']) && ($_POST['goto'] == 'update') ) || (!empty($_GET['mid']) && validate_memberid($_GET['mid']) ) ) {
    ...
    $rsselmember = mysqli_query($conn,"select int_member_id, int_parent_id, int_customer_id, int_designation_id, str_title, dat_last_visit,bit_active, growth, contest, amico_id, miles  from tbl_member WHERE int_member_id = {$memberId}");
    list($memberid,$refer_member_id,$customerid,$designation,$title,$lastvisit,$active, $growth, $contest, $amico_id_xyz, $miles)= mysqli_fetch_row($rsselmember);
    ...
    $sql = "select DISTINCT mem.int_member_id,mem.int_designation_id,mem.amico_id,cus.customers_id,cus.customers_email_address,cus.customers_firstname,cus.customers_lastname, CONCAT(cus.customers_firstname,',',cus.customers_lastname) AS fullname,mem.bit_active
            from tbl_member mem
            inner join customers cus on mem.int_customer_id=cus.customers_id
    ";

    $groupby = " GROUP BY mem.amico_id ";
    ...
    //$conditions[] = "mem.amico_id LIKE 'C%'";
