...
if($cn == '_ppp_') {
	...
	and bw_invoices.InvoiceDate>='".$_3_mo_ago."-01 00:00:00' and bw_invoices.InvoiceDate<='".$_1_mo_ago."-01 00:00:00'";
	...
	FROM bw_invoices, bw_invoice_line_items  
	WHERE bw_invoices.ID='$cv' AND bw_invoices.SKOEInvoice=bw_invoice_line_items.FKEntity 
	and bw_invoices.InvoiceDate<='".$_4_mo_ago."-01 00:00:00' and bw_invoices.InvoiceDate>='".$_6_mo_ago."-01 00:00:00'";
...
if($cn == "_lod_"){
	$cv=$amigo_id;
	$query_lod = " 
	SELECT date_format(o.InvoiceDate, '%m/%d/%Y' ) 
	...
	if ($cv!="N/A") {
	$split=explode("/", $cv);
	$cvudate=$split[2].'-'.$split[0].'-'.$split[1];
	} else {$cvupdate="N/A";};

$updq=mysqli_query($conn,"UPDATE `tbl_member` SET `lod`='$cvudate' WHERE `amico_id`='$amigo_id'") or die (mysql_error());
}
...
$query_current_year = "
	SELECT *  
	FROM bw_invoices, bw_invoice_line_items  
	WHERE bw_invoices.ID='$cv' AND bw_invoices.SKOEInvoice=bw_invoice_line_items.FKEntity AND bw_invoices.InvoiceDate>='".date("Y")."-01-01 00:00:00'";
...
$query_current_year = "
	SELECT *  
	FROM bw_invoices, bw_invoice_line_items  
	WHERE bw_invoices.ID='$cv' AND bw_invoices.SKOEInvoice=bw_invoice_line_items.FKEntity AND bw_invoices.InvoiceDate>='".date("Y")."-".date("m")."-01 00:00:00'";
	$query=mysqli_query($conn,$query_current_year) or die (mysql_error());
	$cv=0;
	while ($f=mysqli_fetch_array($query)) {
        $cv=$cv+$f[ShipQty]*$f[UnitPrice];
...
$query_current_year = "
	SELECT *  
	FROM bw_invoices, bw_invoice_line_items  
	WHERE bw_invoices.ID='$cv' AND bw_invoices.SKOEInvoice=bw_invoice_line_items.FKEntity ORDER BY InvoiceDate asc";
	$query=mysqli_query($conn,$query_current_year) or die (mysql_error());
	...
	while ($f=mysqli_fetch_array($query)) {
        $cv=$cv+$f[ShipQty]*$f[UnitPrice];
	if ($i==0) {$od=$f['InvoiceDate'];};
	$i++;
	}; 

	$split=explode("-", $od);

	$months=round((mktime(0, 0,0, date("m"), date("d"), date("Y"))-mktime(0, 0,0, $split[1], $split[2], $split[0]))/(31*86400));
	if ($months==0) {$months=1;} 
	$cv=round($cv/$months, 2);
