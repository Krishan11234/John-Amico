...
class IncentiveReportGenerator {
    ...
    public function iterateThroughMembers() {
        ...
        /*
        SELECT m.int_member_id, m.amico_id, SUM( (sfo.base_subtotal+sfo.discount_amount) ) AS total_sales FROM tbl_member m INNER JOIN customers c ON m.int_customer_id = c.customers_id INNER JOIN stws_amasty_amorderattr_order_attribute aoa ON m.amico_id = aoa.jareferrer_amicoid INNER JOIN stws_sales_flat_order sfo ON aoa.order_id = sfo.entity_id WHERE m.bit_active=1 AND aoa.jareferrer_self=0 AND aoa.later_applied_member_id IN (0, NULL, '') AND m.amico_id NOT IN ('W00888888') AND sfo.created_at >= '2019-05-11 00:00:00' AND sfo.created_at <= '2019-12-31 11:59:59' GROUP BY m.amico_id ORDER BY m.int_member_id 
        */
    ...
    public function getTopIncentiveReports($limit=3) {
        ...
        if( $this->getPutnaTableReady() ) {
            ...
            /* Added date range */
            $sql .= " AND ir.updated_by_cron >= '{$this->getStartTime()}' AND ir.updated_by_cron <= '{$this->getEndTime()}' ";
