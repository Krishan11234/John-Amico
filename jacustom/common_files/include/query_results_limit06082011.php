                 ...
                 if (is_array($fields_alias)):
					foreach($fields_alias as $key=>$val){
						if ($column_name_display[$i]==$key)
						   ...
						   echo "\t\t<td align=\"$align_column\" class=\"header\" style=\"color:Black;\" nowrap><a href=\"$href&order=$order&camp=$column_name[$i]&l1=0&l2=$l2&$params"."c=1&"."&$get_filter\" class=\"black1\"><b>$column_name_display[$i]</b></a></td>\n";
						}
...
$query_yellow=mysqli_query($conn,$query_yellow) or die (mysql_error());

$query_red=mysqli_query($conn,"SELECT * FROM tbl_member, customers WHERE tbl_member.int_customer_id=customers.customers_id AND tbl_member.amico_id='".$amigo_id."'") or die (mysql_error());
...
$query_red=mysqli_query($conn,"SELECT * FROM tbl_calls WHERE (calldestination='".$member_phone."' OR calldestination='".$member_phone1."' OR calldestination='".$member_phone2."') AND billabletime>'$billabletime' AND calldate<='".date("Y-m-d H:i:s", mktime (0,0,0,date("m"), 31, date("Y")))."' AND calldate>='".date("Y-m-d H:i:s", mktime (0,0,0,date("m"), 1, date("Y")))."'") or die (mysql_error());




if (mysqli_num_rows($query_red)>0) {$font_color="yellow";};
if (mysqli_num_rows($query_yellow)>0)  {$font_color="#00FF00";};

$cv=stripslashes("<a href=\"#\" onClick=\"window.open(\'add_comments.php?b=&mlm_id=".$f['int_customer_id']."',\'comments\',\'scrollbars=yes,width=780,height=400,sizable=1\')\"><font color='".$font_color."'>PC</font></a>");
...
$cv=stripslashes("<a href=\"#\" onClick=\"MM_openBrWindow(\'contact_info2.php?memberid=".$f['int_member_id']."' ,\'info\',\'scrollbars=1,resizable=1,width=450,height=1050\')\"><img src=\"../images/icons/ico_info_red.gif\" width=\"18\" height=\"15\" border=\"0\"></a>");
} else {
$cv=stripslashes("<a href=\"#\" onClick=\"MM_openBrWindow(\'contact_info2.php?memberid=".$f['int_member_id']."' ,\'info\',\'scrollbars=1,resizable=1,width=450,height=1050\')\"><img src=\"../images/icons/ico_info.gif\" width=\"18\" height=\"15\" border=\"0\"></a>");
};
};
...
if($cn == "_ytd2008_"){
//Year To Date
//sum of totals of orders created on year 2008
$query_current_year = "
	SELECT *  
	FROM bw_invoices, bw_invoice_line_items  
	WHERE bw_invoices.ID='$amigo_id' AND bw_invoices.SKOEInvoice=bw_invoice_line_items.FKEntity AND bw_invoices.OrderDate>='2008-01-01 00:00:00' AND bw_invoices.OrderDate<='2009-01-01 00:00:00'";
	$query=mysqli_query($conn,$query_current_year) or die (mysql_error());
	$cv2008=0;
	while ($f=mysqli_fetch_array($query)) {
        $cv2008=$cv2008+$f[ShipQty]*$f[UnitPrice];
	   
	} 
	//echo "UPDATE `tbl_member` SET `ytd2008`='$cv2008' WHERE `amico_id`='$amigo_id'";
	$updq=mysqli_query($conn,"UPDATE `tbl_member` SET `ytd2008`='$cv2008' WHERE `amico_id`='$amigo_id'") or die (mysql_error());
}

if($cn == "_ytd2009_"){
//Year To Date
//sum of totals of orders created on year 2009
$query_current_year = "
	SELECT *  
	FROM bw_invoices, bw_invoice_line_items  
	WHERE bw_invoices.ID='$amigo_id' AND bw_invoices.SKOEInvoice=bw_invoice_line_items.FKEntity AND bw_invoices.OrderDate>='2009-01-01 00:00:00' AND bw_invoices.OrderDate<='2010-01-01 00:00:00'";
	$query=mysqli_query($conn,$query_current_year) or die (mysql_error());
	$cv2009=0;
	while ($f=mysqli_fetch_array($query)) {
        $cv2009=$cv2009+$f[ShipQty]*$f[UnitPrice];
	   
	} 
	//echo "UPDATE `tbl_member` SET `ytd2008`='$cv2009' WHERE `amico_id`='$amigo_id'";
	$updq=mysqli_query($conn,"UPDATE `tbl_member` SET `ytd2009`='$cv2009' WHERE `amico_id`='$amigo_id'") or die (mysql_error());
}
